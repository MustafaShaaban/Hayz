...
class NinjaFormAjaxForm extends FormHandler implements IFormHandler
{
    ...
    {
        SessionUtils::setFormOrFieldId($this->_formSessionVar,$sessionValue);
    ...
    function parseFormDetails()
    {
        ...
        foreach (array_filter($_POST['ninja_ajax_form']['form']) as $key => $value)
        {
            $form[$value]= array(
                ...
                'phone_show'=>$_POST['ninja_ajax_form']['phonekey'][$key],
                'email_show'=>$_POST['ninja_ajax_form']['emailkey'][$key],
                'verify_show'=>$_POST['ninja_ajax_form']['verifyKey'][$key]
            );
        }
        return $form;
    }
