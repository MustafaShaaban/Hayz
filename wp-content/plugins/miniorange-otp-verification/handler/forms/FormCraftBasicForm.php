...
class FormCraftBasicForm extends FormHandler implements IFormHandler
{
    ...
    function validate_formcraft_form_submit()
    {
        ...
        if($otpType===VerificationType::PHONE
            && !SessionUtils::isPhoneVerifiedMatch($this->_formSessionVar,$_POST[$formData['phonekey']])) {
            ...
            && !SessionUtils::isEmailVerifiedMatch($this->_formSessionVar,$_POST[$formData['emailkey']])) {
            $this->sendJSONErrorMessage([
                "errors" => [
        ...
        $this->validateChallenge($otpType,NULL,$_POST[$formData['verifyKey']]);
    }
    ...
    function handleFormOptions()
    {
        ...
        foreach (array_filter($_POST['formcraft_form']['form']) as $key => $value)
        {
            ...
            $form[$value]= array(
                ...
                'phone_show'=>$_POST['formcraft_form']['phonekey'][$key],
                'email_show'=>$_POST['formcraft_form']['emailkey'][$key],
                'verify_show'=>$_POST['formcraft_form']['verifyKey'][$key]
            );
        }
    ...
    private function getFieldIDs($data,$key,$formData)
    {
        ...
        foreach ($formData as $form) {
            if(strcasecmp($form['elementDefaults']['main_label'],$data['formcraft_form']['emailkey'][$key])===0)
                $fieldIds['emailKey']=$form['identifier'];
            if(strcasecmp($form['elementDefaults']['main_label'],$data['formcraft_form']['phonekey'][$key])===0)
                $fieldIds['phoneKey']=$form['identifier'];
            if(strcasecmp($form['elementDefaults']['main_label'],$data['formcraft_form']['verifyKey'][$key])===0)
