...
echo'	<div class="mo_otp_form" id="'.get_mo_class($handler).'"><input type="checkbox" '.$disabled.' id="simplr_default" data-toggle="simplr_default_options" class="app_enable" name="mo_customer_validation_simplr_default_enable" value="1"
				'.$simplr_enabled.' /><strong>'.$form_name.'</strong>';
...
echo'						<div '.($simplr_enabled_type!= $simplr_type_phone ? "hidden" : "").' id="simplr_phone_instruction" class="mo_registration_help_desc">
								...
								<ol>
									...
									<li>'. mo_( "Enter the Field Key of the phone field" ).':<input class="mo_registration_table_textbox" id="mo_customer_validation_simplr_phone_field_key1" name="mo_customer_validation_simplr_phone_field_key" type="text" value="'.$simplr_field_key.'"></li>
								</ol>
							</div>
							</p>
...
echo'							<div '.($simplr_enabled_type != $simplr_type_both ? "hidden" : "").' id="simplr_both_instruction" class="mo_registration_help_desc">
									...
									<ol>
										...
										<li>'. mo_( "In the ShortCode add the following attribute" ).': <b>fields="{Field Key you provided in Step 2}"</b>. '. mo_( "If you already have the fields attribute defined then just add the new field key to the list." ).'</li>
										<li>'. mo_( "Click on <b>update</b> to save your page." ).'</li>
										<li>'. mo_( "Enter the Field Key of the phone field" ).': <input class="mo_registration_table_textbox" id="mo_customer_validation_simplr_phone_field_key2" name="mo_customer_validation_simplr_phone_field_key" type="text" value="'.$simplr_field_key.'"></li>
									</ol>
								</div>
							</p>
						</div>
