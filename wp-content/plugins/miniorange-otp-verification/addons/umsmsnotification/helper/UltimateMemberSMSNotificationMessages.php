...
final class UltimateMemberSMSNotificationMessages extends BaseMessages
{
	...
	private function __construct()
	{
		...
		define("MO_UM_ADDON_MESSAGES", serialize( array(
			...
			self::NEW_UM_CUSTOMER_SMS			    => 	mo_("Thanks for creating an account on {site-name}.".
                                                            "%0aYour username is {username}.%0aLogin Here: ".
                                                            "{accountpage-url}" ),
			self::NEW_UM_CUSTOMER_ADMIN_NOTIF_BODY 	=> 	mo_("Admins are sent a new account SMS notification when".
                                                            " a user signs up on the site."),
			self::NEW_UM_CUSTOMER_ADMIN_SMS 	    => 	mo_('New User Created on {site-name}.%0aUsername: '.
                                                            '{username}.%0aProfile Page: {accountpage-url}'),
		)));
	}
